...
nl_source <- getCtryNlData(
  ...
  nlPeriods = nlRange("201801", "201912"),
...
cn_events$event_score = (8-as.numeric(cn_events$where_prec)) + 
...
# Low precision score: < 19
# Medium precision score: 19-33
# High precision score: >33

# assign levels
cn_events = cn_events %>% 
  mutate(event_precision = case_when(
    .$event_score < 19 ~ "Low",
    .$event_score < 34 ~ "Medium",
    .$event_score > 33 ~ "High",
    TRUE ~ "NONE"
    )
  )
...
```{r PRP}
# Classify events by the severity in deaths from the conflict dataset
...
# Ordered factor, and rename severity levels to a more meaningful ones
...
for (i in 1:nrow(cn_conflicts_list)) {
  ...
  parameter_length = as.numeric((difftime(parameter_end, parameter_start, units="weeks"))/52.25, digits = 2)
...
cn_locations = cn_events %>%
  ...
  filter(event_precision == c("Medium","High")) %>%
...
# model by township with conflicts data
# Select observations where we have population to build the model
md_df <- md_df_pp[md_df_pp$population>0,]

# Create training and test data
md_rowIndex <- sample(1:nrow(md_df), 0.8*nrow(md_df))
md_df_training <- md_df[md_rowIndex, ] # training data
