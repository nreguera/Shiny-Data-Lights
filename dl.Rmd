...
```{r PRP}
# Transform the map into a simplified map
pm = st_as_sf(provinces.map) %>%
...
nl_file <- "E:/Data Lights/Data/nl/NL_DATA_MMR_ADM3_GADM-3.6-SHPZIP.csv"
...
parameter_date_years <- as.numeric(difftime(parameter_endDate, parameter_startDate, unit="weeks"))/52.25
...
# density + quantiles
explore_density <- density(cn_events$date_length)
explore_df <- data.frame(x=explore_density$x, y=explore_density$y)

explore_probs <- c(0.1, 0.25, 0.5, 0.75, 0.9)
explore_quantiles <- quantile(cn_events$date_length, prob=explore_probs)

explore_df$quant <- factor(findInterval(explore_df$x, explore_quantiles))
ggplot(explore_df, aes(x,y)) + 
  geom_line() + 
  geom_ribbon(aes(ymin=0, ymax=y, fill=quant)) + 
  scale_x_continuous(breaks=explore_quantiles) + 
...
for (i in 1:nrow(cn_conflicts_list)) {
  ...
  parameter_length = round(as.numeric((difftime(parameter_end, parameter_start, units="weeks"))/52.25), 2)
...
# calculate the severity in the location
...
```{r EDA}
# Plotting conflicts by severity and region
...
# Map of the selected region with the total_deaths and severity
...
colNames <- c("region", "model")
md_list <- vector(mode = "list", length = 2)
names(md_list) <- colNames

# create models by region
for (i in 1:nrow(rg_list)) {

  # Select observations for each township where we have population to build the model
  md_df_temp <- md_df_pp[md_df_pp$region == rg_list[i,1] &
                      md_df_pp$population > 0,]

  # As some regions haven´t enough observations, we skip them in the model
  if (nrow(md_df_temp) > 2) {
    
    # Split in training and test data
    md_df_index <- sample(1:nrow(md_df_temp), 0.8*nrow(md_df_temp))
    md_df_training <- md_df_temp[md_df_index, ] # training data
    md_df_testing  <- md_df_temp[-md_df_index, ] # test data
    
    # Build the model using training data
    md_lm <- lm(population ~ radiance+area+n_events+c_deaths+t_deaths, data=md_df_training)
    ...
    md_predictions <- predict(md_lm, md_df_testing)
    
    # Attach previous predictions to the real values to measure accuracy
    md_df_preds <- data.frame(cbind(actual=md_df_testing$population, predicted=md_predictions))
    
    # Store the results of the predictions accuracy
    md_lm_evaluation[i,5] <- cor(md_df_preds$actual, md_df_preds$predicted) # corrAccuracy
    md_lm_evaluation[i,6] <- mean(apply(md_df_preds, 1, min) / apply(md_df_preds, 1, max)) # min_max_accuracy 
    md_lm_evaluation[i,7] <- mean(abs((md_df_preds$predicted - md_df_preds$actual))/md_df_preds$actual) # mape
    
    # Append the model to the list
    md_list$region[[i]] <- rg_list[i,1]
    md_list$model[[i]] <- md_lm
...
md_df_results <- data.frame(region=character(),
                            district=character(),
                            township=character(),
                            area=double(),
                            month=integer(),
...
for (i in 1:nrow(rg_list)) {
  ...
  if (!is.null(md_list$model[[i]])) {
    
    # Subset the observations where we don´t have population by region 
    md_df_temp <- md_df_pp[md_df_pp$region == rg_list[i,1] &
                          md_df_pp$population == 0,]
    md_df_temp$population <- NULL
    
    # For each township/period in the region, apply the regional model
    md_predictions <- predict(md_list$model[[i]], md_df_temp)
    
    # attach the predictions column to the temp dataframe
    md_df_temp <- data.frame(cbind(md_df_temp, md_predictions))
  
    # add the current predicted rows to the results dataframe
    md_df_results = data.frame(rbind(md_df_results, md_df_temp))
    
  }

}

# bind the results dataframe to the real observations
md_df_temp = md_df_pp[md_df_pp$population > 0,]

names(md_df_results)[11] <- "population"
md_df_results <- data.frame(rbind(md_df_temp, md_df_results))
...
pp_changes <- pp_changes %>%
    mutate(change = 100 * (population - lag(population))/lag(population))

# assign levels of change
pp_changes = pp_changes %>% 
  mutate(change_level = case_when(
    change < -0.30 ~ "Strong Decrease",
    change < -0.05 ~ "Decrease",
    change < 0.05 ~ "Similar",
    change < 0.30 ~ "Increase",
...
p <- ggplot(pp_temp, aes(month, township, fill=change_level)) +
    geom_tile(color="white", size=0.5) + 
    scale_fill_manual(values = changesPalette) +
#    scale_fill_viridis(name="changes_level", option="E") +
    facet_grid(.~year)
...
cn_file = "E:/Data Lights/Shiny/cn_locations.rda"
save(cn_locations, file=cn_file)

cn_file = "E:/Data Lights/Shiny/cn_conflicts.rda"
save(cn_conflicts, file=cn_file)
...
md_file = "E:/Data Lights/Shiny/md_df_results.rda"
save(md_df_results, file=md_file)

pp_file = "E:/Data Lights/Shiny/pp_changes.rda"
save(pp_changes, file=pp_file)
