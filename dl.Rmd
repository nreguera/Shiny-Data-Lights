...
# From the identifiers columns, we keep region, district, and township
...
```{r PRP2}
# Create a dataset with severity and length of the events

# Subset the events dataframe
columns <- c("id", "type_of_violence", "conflict_id", "date_start", "date_end", "deaths_civilians", "deaths_total", "NAME_1", "NAME_3", "impact_level")
cn_events_explore = cn_events[columns]

# Add a column with the length
cn_events_explore$length = 0
for (i in 1:nrow(cn_events_explore)) {
  
  start = as.Date(cn_events_explore[i, 4])
  end = as.Date(cn_events_explore[i, 5])
  
  cn_events_explore[i, 11] = 
    round(as.numeric(difftime(end, start, unit="weeks"))/52.25,digits = 2)

}

# reorganize the columns
col_order <- c("NAME_1", "NAME_3", "conflict_id", "id", 
               "date_start", "date_end", "length",
               "deaths_civilians", "deaths_total", "impact_level")
cn_events_explore <- cn_events_explore[, col_order]

```


```{r EXP2}
# Creating the exploring viz from previous dataframe
...
ev_filter = cn_events_monthly[which(cn_events_monthly$NAME_1==parameter_region), ]
...
ev_layer = ev_filter[c("id", "latitude", "longitude", "deaths_high", "impact")]
...
for (i in 1:length(st_list)) {
  ...
  nl_temp = nl_data[which(nl_data$region==st_list[[i]]), ]
