...
# Region/state level
rg_geo = ms_simplify(rg_geo, keep = 0.05)

plot(rg_geo[rg_geo@data$NAME_1=="Rakhine",])
plot(rg_geo_simplified[rg_geo_simplified@data$NAME_1=="Rakhine",])

# Township level

# Data and polygons have different row.names
row.names(tw_geo@data)[1:10]
sapply(tw_geo@polygons, slot, "ID")[1:10]

# So first we need to make data slot's row.names match those of the polygons
row.names(tw_geo@data) <- row.names(tw_geo)

# And then we can simplify
tw_geo = ms_simplify(tw_geo, keep = 0.05)

plot(tw_geo[tw_geo@data$NAME_1=="Shan",])
plot(tw_geo_simplified[tw_geo_simplified@data$NAME_1=="Shan",])
...
names(tw_list) <- c("id", "region", "district", "township")                 
...
# it would be interesting to cluster by location and similarity/weight, like
# severity score or conflict id to cluster based on conflicts
# Now we will cluster only based on longitude and latitude, and by region
...
# Prepare de dataset for clustering (longitude, latitude and severity_score)
cn_clusters = cn_events[cn_events$region==parameter_region, c(16,17,30)]

md_mbc = Mclust(cn_clusters)
...
table(cn_temp$severity_level, md_mbc$classification)

# Attach the clusters assigned to each location
cn_clusters = data.frame(cn_temp, cluster=md_mbc$classification)
...
# select numerical variables to have into account for clustering

set.seed(123)
cn_clusters <- data.frame(latitude=numeric(),
                            longitude=numeric(),
                            cluster=numeric(),
                            region=character(),
                            stringsAsFactors=FALSE)
...
for (i in 1:nrow(rg_list)) {
  ...
  # check that there are events in the region
  if (nrow(cn_events[cn_events$region==rg_list[i,1],]) > 0) {
    
    cn_temp = cn_events[cn_events$region==rg_list[i,1], c(16,17)]
    md_dbs = dbscan(cn_temp, eps=0.4, minPts=3)

    cn_temp_clusters = data.frame(cn_temp, cluster=md_dbs$cluster) # cluster
    cn_temp_clusters$region = rg_list[i,1] # region
    cn_temp_clusters$severity_score = cn_events[cn_events$region==rg_list[i,1], 30] # severity_score
    cn_clusters = rbind(cn_clusters, cn_temp_clusters) # add the cluster+region+severity
...
# Plot DBSCAN results
plot(md_dbs, cn_temp_clusters, main = "DBSCAN", frame = FALSE)

# Distribution of points in the clusters (Cluster 0 correspond to outliers)
table(md_dbs$cluster)

# Remove the variables that we don't need anymore
rm(cn_temp, cn_temp_clusters, i)
...
```{r PRP}
# Add aditional information to each cluster

# Severity score of each cluster
cn_clusters <- cn_clusters %>%
  dplyr::group_by(cluster) %>%
  dplyr::mutate(cluster_severity=round(mean(severity_score))) %>%
  as.data.frame()
...
```{r PRP, eval=FALSE}
# Create spatial object to store the clusters polygons by region
# with this code you can use spatial objects

# initialize the sp dataframe where to store the clusters polygons
rg_temp <- SpatialPolygonsDataFrame(SpatialPolygons(list()), data=data.frame())

# turn coordinates into a new polygon
new_polygon <- coordinates %>%
  Polygon() %>%
  list() %>%
  Polygons(ID=j*i) %>%
  list() %>%
  SpatialPolygons(proj4string = CRS(proj4string(tw_geo)))

# assign correct (polygon) ID to the dataframe
#pid <- sapply(slot(new_polygon, "polygons"), function(x) slot(x, "ID")) 
#row.names(cluster_data) = pid

# convert the spatial polygon to spatial polygon dataframe 
polygon_df <- SpatialPolygonsDataFrame(new_polygon, data=cluster_data)

# add the polygon to the region spatial object
rg_temp <- rbind(rg_temp, polygon_df, makeUniqueIDs = TRUE)
...
# initialize the sp dataframe where to store the clusters polygons with the geometry column
rg_clusters <- st_sf(st_sfc(), crs = CRS(proj4string(tw_geo)))

# for each region, create polygons and a dataframe (with severity score, level...)
for (i in 1:nrow(rg_list)) {
  
  # initialize sf variable
  rg_temp <- st_sf(st_sfc(), crs = CRS(proj4string(tw_geo)))
  
  # select the clusters in the region
  cn_temp <- cn_clusters[cn_clusters$region == rg_list[i,1],]
  
  # loop only in those regions that have clusters
  if (nrow(cn_temp[!(cn_temp$cluster==0),])>0) {
    
    # remove those coordinates that aren't in a cluster (cluster=0)
    cn_temp <- cn_temp[!(cn_temp$cluster==0),]
    
    # keep only the coordinates and cluster data in each cluster
    cn_temp_2 <- cn_temp[,c(1,2,3)]
  
    # split the points by cluster (we convert dataframe into a list of dataframes)
    cn_temp_3 <- split(cn_temp_2, cn_temp_2$cluster)
  
    # remove cluster from each list element and keep only the coordinates
    cn_temp_coordinates <- lapply(cn_temp_3, function(x) x[-3])
    
    # compute which points lie on the convex hull (outer points) of each cluster
    # this function returns the index of the point that we will keep, not the coordinates
    cn_temp_chull <- lapply(cn_temp_coordinates, chull)
      
    # keep only those points on the convex hull to draw a polygon across them
    # with this function we get the coordinates from the previous chull index
    cn_temp_outer <- lapply(seq_len(length(cn_temp_chull)), function(x, ...) {
      cn_temp_coordinates[[x]][cn_temp_chull[[x]],]
    })
    
    # create the polygons for the selected clusters in the region
    for (j in 1:length(cn_temp_outer)) {
      
      # select the outer coordinates of each cluster
      coordinates <- cn_temp_outer[[j]]
      
      # check if there are more than 2 coordinates to make a polygon
      if (nrow(coordinates)>2) {
        
        # copy the first point to the end of the dataframe to "close" the polygon
        coordinates <- rbind(coordinates, coordinates[1,])
        
        # turn these coordinates into a polygon
        polygon <- coordinates %>%
          st_as_sf(coords = c("longitude", "latitude"), crs = CRS(proj4string(tw_geo))) %>%
          summarise(geometry = st_combine(geometry)) %>%
          st_cast("POLYGON")
        
        # Add the data to the polygon and convert it to spatial dataframe
        # strategy: as we only have the coordinates and 1 coordinate belongs to only 1 polygon,
        # we will find the first pair of coordinates that match with the polygons
        cluster_data = cn_temp[match(unique(paste0(cn_temp$longitude,cn_temp$latitude)),
                                     paste0(coordinates$longitude,coordinates$latitude)[1]),
                               c(3,4,6)]
        cluster_data = cluster_data[complete.cases(cluster_data),]

        # add the attributes to the polygon
        polygon_df <- st_sf(data.frame(polygon, cluster_data))
        
        # bind the polygons together
        rg_temp <- rbind(rg_temp, polygon_df)
        
      } else {next}
    }
  
    # Add the polygons of the region/state to the sfc
    rg_clusters <- rbind(rg_clusters, rg_temp)

  } else {next}
}

# Remove the variables that we don't need anymore
remove(cn_temp, cn_temp_coordinates, cn_temp_chull, cn_temp_outer, 
       rg_temp, coordinates, i, j,
       cn_temp_2, cn_temp_3)
...
file = "E:/Data Lights/Shiny/rg_clusters.rda"
save(rg_clusters, file=file)
