...
# Prepare de dataset for clustering (only with longitude and latitude)
cn_temp = cn_events[cn_events$region==parameter_region, c(16,17,31)]
cn_temp$severity_level= as.character(cn_temp$severity_level)
cn_temp$severity_level[cn_temp$severity_level == "High"] <- "3"
cn_temp$severity_level[cn_temp$severity_level == "Medium"] <- "2"
cn_temp$severity_level[cn_temp$severity_level == "Low"] <- "1"
cn_temp$severity_level = as.numeric(cn_temp$severity_level)

# select variables to have into account for clustering
# for clustering we must use only numerical variables
# add conflict id to cluster based on conflicts
cn_temp = cn_temp[,-c(3)]
...
md_db <- dbscan(cn_temp, eps=0.4, MinPts=3)

# Attach the clusters assigned to each location
cn_clusters = data.frame(cn_temp, cluster=md_db$cluster)

# Plot DBSCAN results
plot(md_db, cn_temp, main = "DBSCAN", frame = FALSE)

# Distribution of points in the clusters
# Cluster 0 correspond to outliers
table(md_db$cluster)
...
# Prepare de dataset for clustering (only with longitude and latitude)
cn_temp = cn_events[cn_events$region==parameter_region, c(16,17,31)]
d=40000  # Distance threshold         


xy <- SpatialPointsDataFrame(matrix(c(cn_temp$longitude, cn_temp$latitude), ncol=2), data.frame(ID=seq(1:length(cn_temp$longitude))),
